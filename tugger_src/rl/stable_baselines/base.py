...
def _get_save_path(agent_class):
    save_name = "_temp_" + agent_class.__name__ + "_" + str(STEPS)
...
def train_base(agent_class: Callable[..., BaseAlgorithm], policy_class="MlpPolicy"):
    save_path = _get_save_path(agent_class)

    start = time.time()

    env = create_monitor_dummy_vec_env(save_path)
    agent = agent_class(policy_class, env)
